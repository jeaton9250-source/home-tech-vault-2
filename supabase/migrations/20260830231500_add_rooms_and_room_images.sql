create table if not exists public.rooms (
  ...
  constraint rooms_scope_present
    check (
...
with check (
  (
    household_id is not null
    and public.can_household_mutate(
      household_id
    )
  )
  or
...
revoke all
on function
  public.can_read_room_image(text)
from anon;

revoke all
on function
  public.can_mutate_room_image(text)
from anon;
...
using (
  ...
  and public.can_read_room_image(
    name
  )
);
...
with check (
  bucket_id = 'room-images'
  and public.can_mutate_room_image(
    name
  )
);
