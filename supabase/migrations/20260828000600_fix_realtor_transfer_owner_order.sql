...
begin
  ...
  /*
   * IMPORTANT:
   * Change the canonical household owner FIRST.
   *
   * The household_members protection trigger only
   * allows a user to have role='owner' when that
   * user matches households.owner_id.
   */
  ...
  /*
   * Buyer becomes the owner member after the
   * canonical owner pointer has changed.
   */
  ...
  if v_transfer.gift_id is not null then
    ...
    if found then
      ...
      /*
       * Give buyer the remaining gifted plan term.
       */
      if
        v_gift.gift_plan in (
          'pro',
