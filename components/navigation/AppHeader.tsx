...
function usesNewAppShell(pathname: string) {
...
export default function AppHeader() {
  ...
  const { canViewFeature, inheritsFamilyPlan, hasFamilyFeatureAccess } =
    usePermissions();

  const hideDesktopHeader = usesNewAppShell(pathname);
                ...
                {PRIMARY_NAV_ITEMS.map((item) => {
                  ...
                  return (
                    <NavLink
                      ...
                      href={item.href}
                      label={item.label}
                      isActive={isPrimaryNavActive(pathname, item.href)}
                      badge={badge}
                      compact
                    />
                  );
