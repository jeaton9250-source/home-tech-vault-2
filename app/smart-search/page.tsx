...
export default async function SmartSearchPage({
  ...
  if (initialQuery) {
    ...
    if (!error && user) {
      const { householdId, householdOwnerId } = await resolveHouseholdAccess(
        user.id,
        supabase,
      );

      initialResponse = await runSmartSearch({
        supabase,
        userId: user.id,
        householdId,
        householdOwnerId,
  ...
  return (
    <PageShell className="space-y-6 md:space-y-8">
      <header className="space-y-2">
        <p className="text-overline text-text-muted">Ask Your Vault</p>
        ...
        <p className="max-w-2xl text-sm leading-6 text-text-secondary">
          Ask about a device, find a document, check a warranty, review
          maintenance, or search the details your home remembers.
        </p>
      </header>
