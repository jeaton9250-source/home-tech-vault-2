...
export type SmartSearchDeviceRecord = {
  id: string;
...
export type SmartSearchMaintenanceRecord = {
  id: string;
...
export type SmartSearchDocumentRecord = {
  id: string;
  device_id: string | null;
...
export type SmartSearchNetworkRecord = {
  id: string;
...
export type SmartSearchDataSet = {
  devices: SmartSearchDeviceRecord[];
  maintenance: SmartSearchMaintenanceRecord[];
  documents: SmartSearchDocumentRecord[];
  network: SmartSearchNetworkRecord[];
  suggestions?: string[];
  unavailableCategories?: string[];
};

export const DEFAULT_SMART_SEARCH_SUGGESTIONS = [
...
function buildDeviceHaystack(device: SmartSearchDeviceRecord): string {
  return normalize(
    [
      device.device_name,
      device.brand,
...
export function runDeterministicSmartSearch(options: {
  query: string;
  devices: SmartSearchDeviceRecord[];
  maintenance: SmartSearchMaintenanceRecord[];
  documents: SmartSearchDocumentRecord[];
  ...
  if (!intent.normalized) {
    return {
      ...
      total: 0,
      suggestions:
        options.suggestions ?? DEFAULT_SMART_SEARCH_SUGGESTIONS,
    };
  }
  ...
  const hasGenericTerms =
    intent.tokens.length > 0 || intent.phrases.length > 0;
  ...
  for (const device of options.devices) {
    ...
    const locationMatch = matchesLocationHint(
      device.location,
      intent.locationHint
    );
    ...
    if (baseDeviceMatch && !isIntentSpecificWithoutDeviceScope) {
      results.devices.push({
        ...
        match: {
          field: intent.wantsSerialNumber
            ? "Serial Number"
            : "Device",
          value:
    ...
    if (
      !intent.wantsWarrantySoon &&
      !hasDateInFuture(device.warranty_date) &&
  ...
  for (const task of options.maintenance) {
    const deviceName =
    ...
    const needsAttention =
      ...
      (!dueDate ||
        ...
        dueDate.getTime() <= Date.now() + 1000 * 60 * 60 * 24 * 30);
  ...
  for (const document of options.documents) {
    ...
    const haystack = normalize(
      [displayName, document.file_name, document.file_type, deviceName].join(
        " "
      )
    );
  ...
  for (const discovery of options.network) {
    const haystack = normalize(
      [
    ...
    if (
      !intent.wantsNetwork &&
      (!tokenMatch || !phraseMatch || !onlineMatch || !offlineMatch)
    ) {
      continue;
    }
    ...
    results.network.push({
      ...
      title:
        ...
        "Network device",
      subtitle:
        [discovery.manufacturer, discovery.model]
          .filter(Boolean)
          .join(" • ") || null,
      status: discovery.online ? "Online" : "Offline",
  ...
  return {
    ...
    suggestions:
      options.suggestions ?? DEFAULT_SMART_SEARCH_SUGGESTIONS,
    error:
      options.unavailableCategories &&
      options.unavailableCategories.length > 0
        ? `Some results are temporarily unavailable: ${options.unavailableCategories.join(", ")}.`
        : undefined,
  };
}